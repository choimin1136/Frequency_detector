...
TRAIN_BATCH_SIZE = 4
TEST_BATCH_SIZE = 4
TRAIN_REAL_DATAPATH = "tools/realsface"
...
def main(epochs):
    ...
    for i in range(epochs):
        ...
        num = 0
        best_acc = 0.0
        for X_train1, X_train2, X_train3, X_train4, y_train, orimgs in tqdm(get_data(allreal=random.shuffle(os.listdir(TRAIN_REAL_DATAPATH)), allfake=random.shuffle(os.listdir(TRAIN_FAKE_DATAPATH)), batch_size=TRAIN_BATCH_SIZE, device=device)):
            num += 1
            model.train()
            optimizer.zero_grad()
            ...
            if num % 200 == 199:
                ...
                correct = 0
                alldata = 0
                for X_test1, X_test2, X_test3, X_test4, y_test, orimgs in tqdm(get_data(allreal=random.shuffle(os.listdir(TEST_REAL_DATAPATH)), allfake=random.shuffle(os.listdir(TEST_FAKE_DATAPATH)), batch_size=TEST_BATCH_SIZE, device=device)):
